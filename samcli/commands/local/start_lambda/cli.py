...
def cli(ctx,
    ...
    do_cli(ctx, host, port, template, env_vars, authorizer_data, debug_port, debug_args, debugger_path, docker_volume_basedir,
           docker_network, log_file, skip_pull_image, profile, region, parameter_overrides)  # pragma: no cover


def do_cli(ctx, host, port, template, env_vars,
           authorizer_data, debug_port, debug_args,  # pylint: disable=R0914
    ...
    try:
        with InvokeContext(template_file=template,
                           ...
                           authorizer_data_file=authorizer_data) as invoke_context:
